...
The system acts as a complete local pipeline: it handles concurrent web scraping, processes the unstructured data using local Natural Language Processing (NLP) models, and provides an interactive GUI for Exploratory Data Analysis. It was built to replace manual market research with automated, data-driven quantitative insights, enhanced by autonomous AI agents.
...
* **AI & Multi-Agent:** `CrewAI`, `Gemini API`, `FAISS`.
* **Machine Learning (NLP):** `KeyBERT`, `Sentence-Transformers` (`all-MiniLM-L6-v2`), `YAKE`, `TextBlob`.
...
### 2. Multi-Agent AI & RAG Integration
Goes beyond traditional scraping by utilizing Retrieval-Augmented Generation (RAG) and autonomous AI workflows:
* **Agentic Workflows:** Powered by CrewAI and the Gemini API, the system orchestrates specialized agents to perform multi-step reasoning over extracted YouTube data.
* **Vector Search:** Uses FAISS and local HuggingFace embeddings to provide the LLM with instant, cost-effective access to local databases without hitting API token limits.
...
### 3. NLP-Based Semantic Clustering
...
### 4. Exploratory Data Analysis (EDA) Dashboard
...
### 5. Reverse Engineering & Historical Archiving
...
* Successfully integrated local deep learning models and autonomous AI agents into a distributable desktop environment for offline, privacy-first analysis.
